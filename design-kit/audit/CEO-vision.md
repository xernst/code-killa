...
- **What it touches:** New `lib/codemirror-theme.ts` (custom dojoTheme using `--color-ink-*` + `--color-green-*`), `PersistentIDE.tsx:3+255` (replace `oneDark` import + extension), `app/learn/v2/[chapter]/page.tsx:5` and `components/v2/steps/ReadStepView.tsx:5` (delete `import "highlight.js/styles/github-dark.css"`), `app/globals.css` (add `.hljs-*` token mapping per UI Designer §Code-block syntax tokens). Run button gets `bg-green-500 text-ink-950 font-mono uppercase` per UI Designer §Run button. Output `✓ ran in Xms` uses `--ok` token, `text-rose-400` stderr becomes `--err`.
