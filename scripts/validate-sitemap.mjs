...
if (unique.size !== locs.length) fail(`duplicate sitemap URLs found (${locs.length - unique.size} duplicates)`);
for (const url of [
...
for (const url of unique) {
  if (!url.startsWith("https://promptdojo.dev/")) fail(`unexpected sitemap origin: ${url}`);
}
const legacy = [...unique].filter((url) => /\/learn\/(?!v2(?:\/|$))/.test(url));
if (legacy.length > 0) fail(`legacy /learn URLs leaked into sitemap: ${legacy.slice(0, 5).join(", ")}`);
