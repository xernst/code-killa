...
Vaibhav Trivedy (@Vtrivedy10) wrote the cleanest definition of the discipline this curriculum has had to teach: **"Agent = Model + Harness. If you're not the model, you're the harness."** Addy Osmani's May 2026 essay *Agent Harness Engineering* built the whole framing on that line. HumanLayer's writeup the same week landed the corollary: **"It's not a model problem. It's a configuration problem."**
...
- **The harness gap is bigger than the model gap.** Same model in two different harnesses scores radically differently on identical benchmarks. HumanLayer published the receipts: Opus 4.6 placed #33 on Terminal Bench inside Claude Code, then #5 in a different harness. The model didn't change. The scaffolding did.
